...
class Route
{
    ...
    public static function view(string $path, string $template, array $middlewares=[], array $context=[], array $extras=[])
    { return new self($path, [self::class, "renderViewCallback"], ["GET"], $middlewares, array_merge($extras, ["template" => $template, "context" => $context])); }

    public static function renderViewCallback(Request $req)
    {
        $route = $req->getRoute();
    ...
    public static function redirect(string $path, string $target, array $extras=[])
    { return new self($path, [self::class, "redirectRequestToTarget"], [], [], array_merge($extras, ["redirection-target" => $target])); }

    public static function redirectRequestToTarget(Request $req)
    {
        $route = $req->getRoute();
    ...
    public function __invoke(Request $request): mixed
    {
        ...
        {
            // Error message
            Logger::getInstance()->info($err->getMessage());
            throw $err;
        }
    }
}